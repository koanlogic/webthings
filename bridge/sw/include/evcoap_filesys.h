...
ec_filesys_rep_t *ec_filesys_get_rep(ec_filesys_t *filesys,
        const char *uri, ec_mt_t media_type, const uint8_t *etag);
ec_filesys_rep_t *ec_filesys_get_suitable_rep(ec_filesys_t *filesys,
        const char *uri, ec_mt_t *mta, size_t mta_sz, const uint8_t *etag);


/* Resource. */
ec_filesys_res_t *ec_filesys_new_resource(const char *uri, uint32_t max_age);
void ec_filesys_free_resource(ec_filesys_res_t *res);

/* Resource representation. */
int ec_filesys_add_rep(ec_filesys_res_t *res, const uint8_t *data,
        size_t data_sz, ec_mt_t media_type, uint8_t etag[EC_ETAG_SZ]);
